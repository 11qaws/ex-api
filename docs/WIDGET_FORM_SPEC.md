...
증가량 배지는 팔로워가 증가할 때만 오른쪽 위에 나타나는 작은 주황색 종이
조각입니다. 총 적립 결과의 폭은 줄이지 않고 행동 문구가 있는 윗줄의 여유 공간만
사용합니다.

| 항목 | 기본값 |
| --- | ---: |
| 폭 | 내용 너비, 최대 `clamp(72px, 48px × 1.9, 132px)` |
| 최소 높이 | `22px` |
| 위쪽 위치 | `6px` |
| 오른쪽 여백 | `max(10px, 1.2%)` = 기본 `10px` |
| 내부 여백 | 위 `2px`, 좌우 `8px`, 아래 `1px` |
| 기본 회전 | `-0.7deg` |
